...
public class PayBillAdapter extends RecyclerView.Adapter<PayBillAdapter.ViewHolder> {
    ...
    List<PayBillGetter> list;
    Context context;
    Toolbar tb;
    Button checkstatus1;

    public PayBillAdapter(List<PayBillGetter> list, Context context) {
        this.list = list;
    ...
    @NonNull
    @Override
    public PayBillAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.paybilllist,
                parent, false);
        return new PayBillAdapter.ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(PayBillAdapter.ViewHolder holder, int position) {

        PayBillGetter g = list.get(position);
